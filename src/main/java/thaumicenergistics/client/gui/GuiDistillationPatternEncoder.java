...
public class GuiDistillationPatternEncoder extends ThEBaseGui implements IInventoryUpdateReceiver {
			...
			if (slot.getHasStack()) {
				...
				if (aspect != null) {
					...
					GuiParticleAnimator gpa = new GuiParticleAnimator(this.deContainer.slotSourceItem.xDisplayPosition,
							this.deContainer.slotSourceItem.yDisplayPosition, slot.xDisplayPosition,
							slot.yDisplayPosition, 0.3f, EnumGuiParticles.Orb);
	...
	protected void drawGuiContainerBackgroundLayer(final float alpha, final int mouseX, final int mouseY) {
		...
		GuiScreen.itemRender.renderItemAndEffectIntoGUI(this.fontRendererObj, this.mc.getTextureManager(),
				this.thaumometer, -GuiDistillationPatternEncoder.ITEM_HALF_SIZE,
				-GuiDistillationPatternEncoder.ITEM_HALF_SIZE);
	...
	protected void drawGuiContainerForegroundLayer(final int mouseX, final int mouseY) {
		// Draw the title
		this.fontRendererObj.drawString(this.title, GuiDistillationPatternEncoder.TITLE_POS_X,
				GuiDistillationPatternEncoder.TITLE_POS_Y, 0);
	...
	protected void mouseClicked(final int mouseX, final int mouseY, final int mouseButton) {
		...
		for (Slot slot : this.deContainer.slotSourceAspects) {
			if (slot.getHasStack() && ThEGuiHelper.INSTANCE.isPointInGuiRegion(slot.yDisplayPosition,
					slot.xDisplayPosition, 18, 18, mouseX, mouseY, this.guiLeft, this.guiTop)) {
				// Play clicky sound
	...
	public void initGui() {
		...
		this.buttonEncode = new GuiButtonEncodePattern(0,
				...
				GuiDistillationPatternEncoder.BUTTON_ENCODE_POS_Y + this.guiTop, AEStateIconsEnum.STANDARD_ICON_SIZE,
				AEStateIconsEnum.STANDARD_ICON_SIZE);
		this.buttonList.add(this.buttonEncode);

		// Create the reset aspect button
		this.buttonReset = new GuiButtonResetAspectSlot(1,
				GuiDistillationPatternEncoder.BUTTON_RESET_POS_X + this.guiLeft,
				GuiDistillationPatternEncoder.BUTTON_RESET_POS_Y + this.guiTop, AEStateIconsEnum.STANDARD_ICON_SIZE,
				AEStateIconsEnum.STANDARD_ICON_SIZE);
