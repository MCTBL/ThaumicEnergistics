 ...
 *
 */
public class ContainerArcaneAssembler extends ContainerWithNetworkTool {
     ...
     * Upgrade slots.
     */
    private static final int UPGRADE_SLOT_COUNT = BlockArcaneAssembler.MAX_SPEED_UPGRADES, UPGRADE_SLOT_X = 187,
            UPGRADE_SLOT_Y = 26;
    ...
    public ContainerArcaneAssembler(final EntityPlayer player, final World world, final int X, final int Y,
            final int Z) {
        // Call super
        super(player);
        ...
        this.bindPlayerInventory(
                player.inventory,
                ContainerArcaneAssembler.PLAYER_INV_POSITION_Y,
                ContainerArcaneAssembler.HOTBAR_INV_POSITION_Y);
        ...
        this.kCoreSlot = new SlotRestrictive(
                ...
                ContainerArcaneAssembler.KCORE_SLOT_X,
        ...
        for (int row = 0; row < ContainerArcaneAssembler.PATTERN_ROWS; row++) {
            for (int col = 0; col < ContainerArcaneAssembler.PATTERN_COLS; col++) {
                ...
                this.addSlotToContainer(
                        this.patternSlots[index] = new SlotInaccessible(
                                assemblerInventory,
                                invIndex,
        ...
        this.addUpgradeSlots(
                this.assembler.getUpgradeInventory(),
                ContainerArcaneAssembler.UPGRADE_SLOT_COUNT,
                ContainerArcaneAssembler.UPGRADE_SLOT_X,
                ContainerArcaneAssembler.UPGRADE_SLOT_Y);

        // Create the target slot
        this.addSlotToContainer(
                new SlotInaccessible(
                        assemblerInventory,
                        TileArcaneAssembler.TARGET_SLOT_INDEX,
                        ContainerArcaneAssembler.TARGET_SLOT_X,
                        ContainerArcaneAssembler.TARGET_SLOT_Y));

        // Add armor slots
        for (int index = 0; index < ContainerArcaneAssembler.DISCOUNT_ARMOR_COUNT; index++) {
            this.discountSlots[index] = new SlotArmor(
                    assemblerInventory,
                    TileArcaneAssembler.DISCOUNT_ARMOR_INDEX + index,
    ...
    public ItemStack transferStackInSlot(final EntityPlayer player, final int slotNumber) {
        ...
        if ((slot != null) && (slot.getHasStack())) {
                ...
                if (!(slotStack.getItem() instanceof IUpgradeModule)) {
                    ...
                    if (this.kCoreSlot.isItemValid(slotStack)) {
                        didMerge = this.mergeItemStack(
                                slotStack,
                                this.kCoreSlot.slotNumber,
                                this.kCoreSlot.slotNumber + 1,
                                false);
            ...
            // Was the slot clicked the KCore slot or armor slots?
                    ...
                    || ((slotNumber >= this.discountSlots[0].slotNumber) && (slotNumber
                            <= this.discountSlots[ContainerArcaneAssembler.DISCOUNT_ARMOR_COUNT - 1].slotNumber))) {
